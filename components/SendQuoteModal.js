...
import { buildQuotePdfBlob, blobToBase64 } from "@/lib/pdf";

export default function SendQuoteModal({ quote, customer, onClose }) {
  ...
  async function handleSend(e) {
    ...
    try {
      ...
      const quoteLink = `${window.location.origin}/q/${quote.id}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-xl border border-border bg-surface p-6">
        <h2 className="text-lg font-semibold text-ink">Email Quote to Customer</h2>
        <p className="mt-1 text-sm text-muted">
          Sends {quote.number} as a PDF attachment, with a link to view/accept it online.
        </p>
          ...
          <form onSubmit={handleSend} className="mt-4 space-y-3">
            ...
            <label className="block">
              ...
              <textarea
                ...
                rows={3}
                placeholder="A quick note to include above the quote details…"
                className="mt-1 w-full rounded-lg border border-border bg-surface-2 px-3 py-2 text-sm text-ink outline-none focus:border-accent"
              />
            </label>
