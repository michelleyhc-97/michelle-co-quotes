...
const CONTENT_IDEAS_SCHEMA = {
  type: "ARRAY",
  items: { type: "STRING" },
  description: "Exactly 3 short, specific, ready-to-pitch content ideas for this service.",
};

/** A few quick content ideas for a specific service, shown to a customer
 * in the Telegram bot right after they tap it — a small inspiration
 * nudge, not anything that affects pricing or the order itself. Returns
 * null if GEMINI_API_KEY isn't set or the call fails, so the bot can just
 * skip straight to asking for quantity instead of blocking the order on
 * this being unavailable. */
export async function generateContentIdeas(serviceName) {
  const ai = getClient();
  if (!ai) return null;

  const prompt = `You are a creative director at Michelle & Co. Creatives, a content & copywriting studio in Malaysia. A customer just picked the service "${serviceName}". Suggest exactly 3 short, specific, ready-to-pitch content ideas for this service — concrete enough to picture and act on, not generic advice like "post more often". One sentence each.`;
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: CONTENT_IDEAS_SCHEMA,
      },
    });
    return JSON.parse(response.text);
  } catch (err) {
    console.error("generateContentIdeas failed:", err);
    return null;
  }
}
